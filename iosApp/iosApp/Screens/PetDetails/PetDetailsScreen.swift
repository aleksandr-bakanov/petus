...
struct PetDetailsScreen: View {
    ...
    init(petId: Int64, isShowingDetail: Binding<Bool>) {
        self.petId = petId
    ...
    var body: some View {
        NavigationView {
            ScrollView { // Like a Column in Compose with scrolling
                VStack(alignment: .leading, spacing: 16) {
                    ActionButton(title: "Close", backgroundColor: .accentColor) {
                        isShowingDetail = false
                    }
                    
                    VStack(spacing: 0) {
                        if !viewModel.uiState.petImage.isEmpty {
                            Image(viewModel.uiState.petImage) // Assuming SwiftUI loads images by name
                                .resizable()
                                .scaledToFit()
            ...
            .navigationBarTitleDisplayMode(.inline) // Or .large if you want bigger title
        }
        .task {
